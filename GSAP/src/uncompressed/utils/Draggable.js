...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	window._gsDefine("utils.Draggable", ["events.EventDispatcher","TweenLite"], function(EventDispatcher, TweenLite) {
		...
		var _tempVarsXY = {css:{}}, //speed optimization - we reuse the same vars object for x/y TweenLite.set() calls to minimize garbage collection tasks and improve performance.
			...
			_point2 = {},
			/* convenience methods that we may use later, but don't need yet...
			_get2DMatrix = function(e, includeOffset) {
				return includeOffset ? _getOffset2DMatrix(e, _getOffsetTransformOrigin(e, _point1), _getOffsetTransformOrigin(e.parentNode, _point2)) : _getOffset2DMatrix(e);
			},
			_placeElementAt = function(e, p, context) {
				var offset = _getOffsetOrigin(e);
				context = context || e.parentNode;
				p = _localToGlobal(context, p);
				_globalToLocal(e.parentNode, p, true);
				TweenLite.set(e, {x: p.x - offset.x, y: p.y - offset.y});
			},
			_getOffsetOrigin = function(e, decoratee) {
				var origin = _getOffsetTransformOrigin(e, _point1),
					parentOrigin = _getOffsetTransformOrigin(e.parentNode, _point2);
				decoratee = decoratee || {};
				decoratee.x = origin.x + e.offsetLeft - parentOrigin.x;
				decoratee.y = origin.y + e.offsetTop - parentOrigin.y;
				return decoratee;
			},
			_globalToLocal = function(e, p, decoratee) {
				var m = _getConcatenatedMatrix(e, true),
					x = p.x,
					y = p.y;
			...
			*/
			...
			_getOffset2DMatrix = function(e, offsetOrigin, parentOffsetOrigin) {
				...
				if (offsetOrigin) {
					...
					if (_hasReparentBug === undefined && _doc.body) {
